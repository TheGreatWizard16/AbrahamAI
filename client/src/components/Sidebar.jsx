...
const Sidebar = ({ sidebar, setSidebar }) => {
  ...
  const { signOut, openUserProfile } = useClerk(); // Clerk functions for sign out and profile

  return (
    // Sidebar container with responsive behavior for small screens
    <div
    ...
    >
      {/* Top section: user avatar and navigation links */}
        ...
        <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
          {navItems.map(({ to, label, Icon }) => (
            <NavLink
              key={to}
              to={to}
              end={to === '/ai'} // Make Dashboard link exact
              onClick={() => setSidebar(false)} // Close sidebar on link click (for small screens)
              className={({ isActive }) =>
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon className={`w-4 h-4 ${isActive ? 'text-white' : ''}`} /> {/* Icon color changes if active */}
                  {label}
                </>
              )}
            </NavLink>
