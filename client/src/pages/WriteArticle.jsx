...
const WriteArticle = () => {
  ...
  // Currently selected article length; default is the first option
  const [selectedLength, setSelectedLength] = useState(articleLength[0])

  // User input for the article topic
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      // Important fix: call getToken() instead of getToken
      ...
      // Fix: backend sends "success", not "succes"
      if (data.success) {
      ...
      // Fix: data is not available here, use error.response safely
      ...
      >
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {articleLength.map((item, index) => (
            // Clicking an option updates selectedLength; selected option gets highlighted
            <span
        ...
        {/* Submit button to generate article */}
        <button 
          disabled={loading} 
