...
// Set base URL for Axios from environment variables
...
const GenerateImages = () => {
  ...
  const [selectedStyle, setSelectedStyle] = useState('Realistic'); // selected style
  const [input, setInput] = useState(''); // image description
  const [publish, setPublish] = useState(false); // public toggle
  ...
  const onSubmitHandler = async (e) => {
    e.preventDefault();
    if (!input) return toast.error('Please enter a description for the image.');
    ...
    try {
      ...
      if (data.success) {
        ...
        // UX feedback
        if (publish && data.created) {
          toast.success('Your image is now public and will appear in the Community!');
        } else {
      ...
      <div className="w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96">
        ...
        {!content ? (
          ...
          <div className="mt-3 h-full">
            <img src={content} alt="Generated result" className="w-full h-full object-contain" />
          </div>
        )}
      </div>
    </div>
